...
then
    ...
    celery -A processor worker -B -s ${CELERY_BEAT_SCHEDULE_DIR}/celerybeat-scheduler -l ${CELERY_LOG_LEVEL}
elif [ "${PROCESSOR_TYPE}" == "BEAT" ] ;
then
    echo "Beat processor started"
    celery -A processor beat -s ${CELERY_BEAT_SCHEDULE_DIR}/celerybeat-scheduler -l ${CELERY_LOG_LEVEL}
